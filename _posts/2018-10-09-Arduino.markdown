...
In questo semplice progetto le componenti sono fondamentalmente 2: un clone **Arduino Uno Rev. 3** ed un clone **Adafruit NeoPixel Ring** a 24 LEDs. Sebbene la seconda sia identica sotto ogni punto di vista all’originale, la board in questione presenta invece una sola differenza: il modulo di comunicazione seriale (USB to Serial). Parliamo infatti di un CH340G, prodotto dalla casa cinese WCH, molto più economico del FT232RL montato sull’originale. Sono stato quindi costretto a scaricare i driver dal sito WCH. Questi si presentano in varie sezioni:
...
Difatti, è scrivere `uint32_t` è come scrivere `unsigned int` nello stesso IDE. Ma qual è la differenza tra tra una variabile e la stessa variabile non firmata (**unsigned**)?

Prendiamo di nuovo come esempio int. Essendo una variabile a 32 bit il range di valori integer occupabili va da −2,147,483,648 a 2,147,483,647. Difatti questa variabile prende il nome di **int** o **int firmata** (**signed int**). Per quanto riguarda **unsigned int** lo stesso valore a 32 bit viene considerato solo in valori positivi, per cui in un range da 0 a 4,294,967,295.

Nel nostro caso Adafruit consiglia di usare valori unsigned poiché non lavorando in campo di valori negativi (un pin non può assumere un valore negativo) allora è bene sfruttare al massimo il campo degli integer tramite dei semplici **integer** positivi, e quindi variabili positive (e non firmate).
...
```
Quest’ultima è il ciclo più complesso. Ritroviamo infatti un sottociclo per il valore q tramite cui otteniamo il theater-chase, e all’interno di esso un ulteriore sotticiclo for che quindi somma il valore di `i` a *3*, somma nel sottociclo successivo `i` a `q` e tramite la funzione `Ruota` assegna il colore.
...
![alt text](/img/in-post/post-js-version/Ring_schem.png "Schematica")
...
![alt text](/img/in-post/post-js-version/Ring_bb.png "BreadBoard")
